...
# # error - Book#display data is not available to a Library object
# #         The method should be in the Library class since we are trying
# #         to display library objects. Need to call a Book method since we are
# #         iterating over book objects
...
#     super(diet, superpower) # missing arguments on super which was causing error when SongBird object is initialized
#     @song = song
#   end
...
# # FligthlessBird#initialize is not necessary since the same arguments are being passed up the method look up chain
...
# # returns false because these are two separate objects that happen to have the same value. but they are not the same objects
# # need to override the Object method and define a custom method that compares values and not object ids
...
# # 1 mi
# # 1 nmi
# # needs a custom <=> method for sort to work
...
#       # needed to give the variable a new name so ruby wouldn't confuse the new variable
