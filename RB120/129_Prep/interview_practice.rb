...
module Flightable
  def fly
    fly = Ability.new("fly").description
...
class Superhero
  include Flightable
  attr_accessor :ability
  attr_reader :name # I added this line... didn't see how to access name getter in Flightable without it
  def self.fight_crime
    puts "I am #{self}!"
    Superhero.new("").announce_ability
  end
  ...
  def announce_ability
    puts "I fight crime with my #{Ability.new("coding skills").description} ability!"
  end
end
...
# Can only change 2 methods where you see #IMPLEMENTATION . I want you to use the Ability class. superman.fly and
# LSMan.fight_crime  should produce the commented outputs below them and the words LSMan, Superman, superhero, fly and
# coding skills should be interpolated.
