...
item manager
  - responsible for creating items, updating information about items, deleting items, and querying information about the items
reports manager
  ...
  - takes an ItemManager object as its argument for init
...
let ItemManager = {
  ...
  create(itemName, category, quantity) {
    function validItem() {
      if (quantity === undefined || !['sports', 'cooking'].includes(category) || itemName === 'asd') {
        // return {
        //   notValid: true,
        // }
        return false
  ...
  itemsInCategory(cat) {
    console.log(this.items.filter(obj => obj.category === cat));
  },
  ...
  createReporter(sku) {
    ...
    return {
      itemInfo() {
        ...
        let item = self.obj.items.find(o => o.skuCode === sku);
